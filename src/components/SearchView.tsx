...
const globalChannels = [
  { name: 'Привет, не хочешь сход…', sub: '@privetpoidem, 58 918 subscribers', bg: 'linear-gradient(135deg,#f7d44c,#e8b321)', t: 'П' },
  { name: 'Привет, Москва!', sub: '@progorodmsk, 77 875 subscribers', bg: '#ffffff', t: 'M', tc: '#e0322a' },
  { name: 'Привет, Аутлет!', sub: '@bestoffersmoscow, 45 657 subs…', bg: 'linear-gradient(135deg,#1f2a1f,#0a0a0a)', t: '%', tc: '#5cc85e', verified: true },
]
const channelsList = [
                ...
                {active && (
                  <motion.div
                    ...
                    style={{
                      ...
                      borderRadius: 14,
                      background:
                        theme.palette.mode === 'dark' ? 'rgba(135,116,225,0.22)' : 'rgba(125,99,232,0.14)',
                    }}
                  />
                )}
                <Typography
...
function ResultRow({ bg, t, tc, title, subtitle, verified, tg }: any) {
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, px: 1, py: 0.65, borderRadius: '12px', cursor: 'pointer', '&:hover': { background: tg.hover } }}>
      <Avatar background={bg} text={t} size={48} color={tc ?? '#fff'} />
      <Box sx={{ flex: 1, minWidth: 0 }}>
  ...
  return (
    <Box onClick={() => onSelect(m.id)} sx={{ display: 'flex', alignItems: 'center', gap: 1.5, px: 1, py: 0.65, borderRadius: '12px', cursor: 'pointer', '&:hover': { background: tg.hover } }}>
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
          <PlayArrowRounded sx={{ fontSize: 18, color: tg.textSecondary }} />
          <Typography noWrap sx={{ fontSize: 15, color: tg.textPrimary }}>
            <Highlighted text={m.text} q={query} color={tg.accent} />
          </Typography>
        </Box>
