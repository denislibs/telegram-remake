...
export default function ChatListItem({ chat, selected, onClick, index = 0 }: Props) {
  ...
  const menuItems: { icon: ReactNode; label: string; danger?: boolean }[] = [
    { icon: <AddBoxOutlined />, label: 'Open in new tab' },
    { icon: <VisibilityOutlined />, label: 'Preview' },
    { icon: <MarkChatUnreadOutlined />, label: 'Mark as unread' },
    { icon: <PushPinOutlined />, label: 'Pin' },
    {
      icon: chat.muted ? <NotificationsNoneOutlined /> : <NotificationsOffOutlined />,
      label: chat.muted ? 'Unmute' : 'Mute',
    },
    { icon: <ArchiveOutlined />, label: 'Archive' },
    { icon: <DeleteOutlineRounded />, label: destructive, danger: true },
  ]
    ...
    >
      ...
      <Box sx={{ minWidth: 0, flex: 1 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mt: 0.25 }}>
          {chat.sent && (
            <DoneRoundedIcon
              sx={{ fontSize: 17, color: onAccent ? '#fff' : tg.accent, flexShrink: 0 }}
            />
          )}
          <Typography
            noWrap
